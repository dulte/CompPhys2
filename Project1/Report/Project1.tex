...
\begin{document}
	...
	\end{equation}
	Where $F_i$ is the quantum force on particle $i$ and $D$ is the diffusion parameter, discussed in section \ref{sec:Metropolis_Hasting_Sampling}. The parameter $dt$ is the same parameter used in the Greens function (equation \ref{eq:Greens_Function}), and we find it by looking at the energy for the exactly solvable system for different $dt$. We find that we get good results for $dt=0.05$, as described in section \ref{sec:finding_optimal_step_size} and \ref{sec:Disc_importance_sampling_step_size}.
	...
	\pagebreak	
		\begin{figure}[ht!]
			\centering
				...
				\end{subfigure}
			\caption{Local energy for various values of $\alpha$, for $a=0$, $\beta=1$, $\omega_z=1$, $N=100$ in 3 dimensions using $2^{20}$ Monte Carlo cycles per core. This is done with both brute force and importance sampling.}
		\end{figure}
	\linebreak
