...
\begin{document}
	...
	\begin{appendices}
		...
		\end{equation}
		Where $a_{ho}=(h/m\omega)^{1/2}$. Note that the Laplacian under this transformation turns into:
		\begin{equation}
		\nabla^2 \rightarrow \frac{\hbar}{m\omega} \nabla^2 
		\end{equation}
		Thus, performing this coordinate transformation gives:
		\begin{equation}
		H \rightarrow H =\frac{\hbar \omega}{2} \left(\sum_{i=1}^N -\nabla_i^2   + x_i^2+y_i^2+\frac{\omega_z^2}{\omega^2} z^2\right)+\sum_{i<j}^N V_{int}(\boldsymbol{r}_i, \boldsymbol{r}_j)
		\end{equation}
		Now use that $H\Psi_T=E\Psi_T$, and rescale the energy as $E/\hbar \omega$. This gives:
		\begin{equation}
		\frac{1}{\hbar \omega}H\Psi = \frac{1}{2}\sum_{i=1}^N\left(-\nabla^2_i +x_i^2+y_i^2+  \frac{\omega_z^2}{\omega^2} z_i^2\right)+\sum_{i<j}^N V_{int}(\boldsymbol{r}_i, \boldsymbol{r}_j) = \frac{E}{\hbar \omega}\Psi
		\end{equation}
		Relabeling the energy and defining $\gamma=\omega/\omega_z$ gives:
		\begin{equation}
		H=\frac{1}{2}\sum_{i=1}^N \left(-\nabla_i^2 +x_i^2+y_i^2+\gamma^2 z_i^2\right)+\sum_{i<j}^N V_{int}(\boldsymbol{r}_i, \boldsymbol{r}_j)
		\end{equation}
